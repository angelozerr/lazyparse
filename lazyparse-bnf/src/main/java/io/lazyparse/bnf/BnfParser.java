...
public class BnfParser {
    ...
    private Rule parseRule() {
        System.out.println("Parsing rule at token " + peek().type + "('" + peek().text + "') at position " + peek().position);

        CharBnfLexer.Token nameToken = consume();
        if (nameToken.type != CharBnfLexer.TokenType.IDENTIFIER) {
            throw new RuntimeException("Expected IDENTIFIER but found " + nameToken.type + " at position " + nameToken.position);
        }
    ...
    private Expression parseSequence() {
        ...
        while (true) {
            ...
            if (current.type == TokenType.PIPE ||
                ...
                current.type == TokenType.COLON_COLON_EQUALS ||
    ...
    private Expression parsePrimary() {
        ...
        if (t.type == CharBnfLexer.TokenType.ANNOTATION) {
            ...
            /*CharBnfLexer.Token annotationNameToken = consume();
            if (annotationNameToken.type != CharBnfLexer.TokenType.IDENTIFIER) {
                throw new RuntimeException("Expected IDENTIFIER after '@' but found " + annotationNameToken.type);
            }*/
           ...
           // Expression expr = parsePrimary();
            ...
            if (argument != null) {
                ...
                elems.add(argument);
               // elems.add(expr);
                return new Annotation(annotationName, new Sequence(elems));
            } else {
    ...
    public static class Sequence implements Expression {
    ...
    public static class Alternative implements Expression {
    ...
    public static class Repetition implements Expression {
        public enum Type {
    ...
    public static class Annotation implements Expression {
        public final String name;
        public final Expression expr;
